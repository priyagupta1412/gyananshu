...
switch ($search_category) {
    case 'faculty_name':
        $sql = "SELECT faculty.faculty_name, faculty.designation, faculty.expertise, department.department_name FROM faculty JOIN department ON faculty.department_id = department.department_id WHERE faculty.faculty_name LIKE '%$search_query%'";
        break;
    case 'designation':
    ...
    case 'department':
        ...
        break;
    case 'publication':
        $sql = "SELECT faculty.faculty_name, publication.publication_title FROM faculty JOIN publication ON faculty.faculty_id = publication.faculty_id WHERE publication.publication_title LIKE '%$search_query%'";
        break;
    case 'conference':
        $sql = "SELECT faculty.faculty_name, conference.conference_name  FROM faculty JOIN conference ON faculty.faculty_id = conference.faculty_id WHERE conference.conference_name LIKE '%$search_query%'";
        break;
    default:
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        switch ($search_category) {
            ...
            case 'designation':
            case 'expertise':
                echo "Name: " . $row["faculty_name"] . " - Designation: " . $row["designation"] . " - Expertise: " . $row["expertise"] . "<br>";
                break;
            case 'department':
                echo "Name: " . $row["faculty_name"] . " - Department: " . $row["department_name"] . "<br>";
                break;
            case 'publication':
                echo "Faculty Name: " . $row["faculty_name"] . " - Publication Title: " . $row["publication_title"] . "<br>";
                break;
            case 'conference':
                echo "Faculty Name: " . $row["faculty_name"] . " - Conference Name: " . $row["conference_name"] . "<br>";
                break;
        }
    }
} else {
